                             ...
                             const char *average_method) // loc, har, ari
{
    ...
    /*Read interface file*/
    read_interface_file(in_var_file, &interfaces, media_type);
  
    if (media_type != ONE_COMPONENT) {
        fprintf(stderr, "Error: media_type=%s is not supported,\n"\
                ...
                md_map[media_type].c_str(), in_var_file);   
        fflush(stderr);
        exit(1);
    }

    if (strcmp(average_method, "loc") == 0) {
        parametrization_layer_onecmp_loc(nx, nz, x2d, z2d, grid_type, media_type, interfaces, var2d);
    } else if (strcmp(average_method, "har") == 0) {      // harmonic
        parametrization_layer_onecmp_har(nx, nz, x2d, z2d, grid_type, media_type, interfaces, var2d);
    } else if (strcmp(average_method, "ari") == 0) {      //arithemtic
        parametrization_layer_onecmp_ari(nx, nz, x2d, z2d, grid_type, media_type, interfaces, var2d);
    } else {                                                        // default = loc
        ...
        const char *equivalent_medium_method) 
{
    ...
    /*Read interface file*/
    read_interface_file(in_lay_file, &interfaces, media_type);

    if (media_type != ACOUSTIC_ISOTROPIC) {
        fprintf(stderr, "Error: media_type=%s is not supported,\n"\
                        "       media_layer2model_ac_iso() only supports acoustic_isotropic,\n"\
                        "       please check the media_type in %s! \n", 
                md_map[media_type].c_str(), in_lay_file);         
        fflush(stderr);
        exit(1);
    }

    if (strcmp(equivalent_medium_method, "loc") == 0) {
        parametrization_layer_ac_iso_loc(nx, nz, x2d, z2d, grid_type, 
            media_type, interfaces, kappa2d, rho2d);
    } else if (strcmp(equivalent_medium_method, "ari") == 0 ) {
        parametrization_layer_ac_iso_ari(nx, nz, x2d, z2d, grid_type, 
            media_type, interfaces, kappa2d, rho2d);
    } else if (strcmp(equivalent_medium_method, "har") == 0 ) {
        parametrization_layer_ac_iso_har(nx, nz, x2d, z2d, grid_type, 
            media_type, interfaces, kappa2d, rho2d);
    } else { 
        ...
        const char *equivalent_medium_method) // 
{
    ...
    if (media_type != ELASTIC_ISOTROPIC) {
        fprintf(stderr, "Error: media_type=%s is not supported,\n"\
    ...
    if (strcmp(equivalent_medium_method, "loc") == 0) {
        parametrization_layer_el_iso_loc(nx, nz, x2d, z2d, grid_type, 
            media_type, interfaces, lam2d, mu2d, rho2d);
    } else if (strcmp(equivalent_medium_method, "har") == 0) {
        parametrization_layer_el_iso_har(nx, nz, x2d, z2d, grid_type, 
            media_type, interfaces, lam2d, mu2d, rho2d);
    } else if (strcmp(equivalent_medium_method, "ari") == 0) {
        parametrization_layer_el_iso_ari(nx, nz, x2d, z2d, grid_type, 
            media_type, interfaces, lam2d, mu2d, rho2d);
    } else { //default
        ...
        const char *equivalent_medium_method) 
{
    ...
    if (md_type != ELASTIC_VTI_PREM && md_type != ELASTIC_VTI_THOMSEN && md_type != ELASTIC_VTI_CIJ) {
        fprintf(stderr, "Error: media_type=%s is not supported in media_layer2model_el_vti(),\n"\
                        ...
                        "       please check the media_type in %s! \n", 
                md_map[md_type].c_str(), in_lay_file);      
        fflush(stderr);
        exit(1);
    }

    if (strcmp(equivalent_medium_method, "loc") == 0) {
        parametrization_layer_el_vti_loc(nx, nz, x2d, z2d, grid_type, 
            md_type, interfaces, c11, c33, c55, c13, rho);
    } else if (strcmp(equivalent_medium_method, "har") == 0) {
        parametrization_layer_el_vti_har(nx, nz, x2d, z2d, grid_type, 
            md_type, interfaces, c11, c33, c55, c13, rho);
    } else if (strcmp(equivalent_medium_method, "ari") == 0) {
        parametrization_layer_el_vti_ari(nx, nz, x2d, z2d, grid_type, 
            md_type, interfaces, c11, c33, c55, c13, rho);
    } else { //default
        ...
        const char *equivalent_medium_method) 
{
    ...
    if (md_type == ELASTIC_ISOTROPIC) {
        if (strcmp(equivalent_medium_method, "loc") == 0) {
            parametrization_layer_el_iso_loc(nx, nz, x2d, z2d, grid_type, 
                md_type, interfaces, c13, c55, rho);
            ...
            parametrization_layer_el_iso_tti(nx, nz, x2d, z2d, grid_type,
                md_type, interfaces, rho, c11, c13, c15, c33, c35, c55);
        } else {
    ...
    // vti: loc, har, ari    
        ...
        if (strcmp(equivalent_medium_method, "loc") == 0) {
           ...
           parametrization_layer_el_vti_tti(nx, nz, x2d, z2d, grid_type,
                md_type, interfaces, rho, c11, c13, c15, c33, c35, c55);
        } else {
            fprintf(stderr, "Error: no such equivalent_medium_method: %s! \n", equivalent_medium_method);        
            fflush(stderr);
    ...
    float *var2d) 
{
    ...
    parametrization_layer_onecmp_loc(nx, nz, Gridx, Gridz,
        grid_type, media_type, interfaces, var2d);
    ...
    MarkInterfaceNumber(grid_type, Hx, Hz, 
        nx, nz, MaterNum, interfaces);
    ...
    for (size_t k = 1; k < nz-1; k++) {
        for (size_t i = 1; i < nx-1; i++) {
            ...
            if ( NumOfValues(v, NI) > 1) {
                ...
                var2d[indx] = num_dis*1.0/vol_var;
    ...
    for (size_t k = 1; k < nz-1; k++) {
        for (size_t i = 1; i < nx-1; i++) {
            ...
            if ( NumOfValues(v, NI) > 1) {
                ...
                for (size_t isg = 0; isg < nsg; isg++) {
                    std::vector<float> var(1, 0.0);
                    int isPointOnInter = AssignLayerMediaPara2Point(
                        i, k, SubGrid[isg], interfaces, ONE_COMPONENT, var);
    ...
    float *rho2d) 
{
    ...
    // assign the local value first.
    parametrization_layer_ac_iso_loc(nx, nz, Gridx, Gridz,
        grid_type, media_type, interfaces, kappa, rho2d);
    ...
    for (size_t k = 1; k < nz-1; ++k) {
        for (size_t i = 1; i < nx-1; ++i) {
            ...
            if ( NumOfValues(v, NI) > 1) {
                ...
                rho2d[indx] = ari_rho/(1.0*num_dis);
    ...
    for (size_t k = 1; k < nz-1; k++) {
        for (size_t i = 1; i < nx-1; i++) {
            ...
            if ( NumOfValues(v, NI) > 1) {
                ...
                rho2d[indx] = ari_rho/(1.0*num_dis);
                kappa[indx] = ari_kappa/(1.0*num_dis);
...
//- elasic isotropic
// ref: Moczo, 2002, 2d Heterogeneous Staggered-Grid Finite-Difference Modeling of Seismic
    ...
    float *rho2d) 
{
    ...
    // assign the local value first.
    parametrization_layer_el_iso_loc(nx, nz, Gridx, Gridz,
        grid_type, media_type, interfaces, lam2d, mu2d, rho2d);
...
{
    size_t siz_slice = nz * nx;
    int NI = interfaces[0].NI;    
    
    // assign the local value first.
    parametrization_layer_el_vti_loc(nx, nz, Gridx, Gridz,
        grid_type, media_type, interfaces, c11, c33, c55, c13, rho);
    ...
    float *c55) 
{
    ...
    parametrization_layer_el_iso_har(nx, nz, Gridx, Gridz,
        grid_type, media_type, interfaces, c13, c55, rho);
            ...
            if (NumOfValues(v, NI) == 2) {
                ...
                if (intersectionList.size() == 2) {
                    ...
                    for (int isg = 0; isg < nsg; isg++) {
                        ...
                        // ignore the sub-point that on the interface
                        if (isPointOnInter == 1)
                            num_dis--;
                        else {
                            float vp = var[1], vs = var[2], rho = var[0];
    ...
    float *c55) 
{
    ...
    // assign the local value by har first 
    // (can just consider the numofvalue == 2).
    parametrization_layer_el_vti_har(nx, nz, Gridx, Gridz,
        grid_type, media_type, interfaces, c11, c33, c55, c13, rho);
    for (int i = 0; i < siz_slice; i++) {
        c15[i] = 0.0;
        c35[i] = 0.0;
